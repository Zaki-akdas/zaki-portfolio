...
The site is a Next.js 14 app with a **JSON-file content store** (`data/`).
Media uploads live in **Supabase Storage** (public `media` bucket) — no
writable disk is needed for uploads. Messages and rate limits are durable in
Supabase Postgres when `DATABASE_URL` is set. The recommended host is
...
| `DATABASE_URL` | Supabase pooler string | Durable messages + rate limits (Postgres) |
